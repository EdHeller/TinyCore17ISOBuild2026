...
def fetch(item):
  # return if no url
  if item == '':
	   #print('BUG ALERT! No URL', item)
	   return
  
  # substitute KERNEL
  if 'KERNEL' in item:
    # for 32-bit version use '5.15.10-tinycore'
    item = item.replace('KERNEL', '6.18.2-tinycore')
  ...
  # download package
  if download(tcz, 'wb') == 2: 
	  #print('something about wb?', dep)
	  return
...
def check_deps(item):
  #print('BUG ALERT! check deps', item)
  deps_tree = requests.get(MIRROR + item + '.tree')
  
    
  if '<html>' in deps_tree.text:
	  return
...
MIRROR = 'https://distro.ibiblio.org/tinycorelinux/17.x/x86/tcz/'
...
#sh('file ./download.lst')
# init downloads
with open('./download.lst') as f:
    downloads = f.read().split('\n')[:-1]
    for things in downloads:
        fetch(things)
